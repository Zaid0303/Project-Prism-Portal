...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($response_data && isset($response_data['join_url'])) {
        $join_url = $response_data['join_url'];
        
        // Store the meeting URL in the database
        $meeting_topic = $_POST['topic'];
        $meeting_start_time = $_POST['start_time'];
    
        // Assuming you have a connection and table ready for storing this data
        $store_meeting_query = "INSERT INTO meetings (topic, join_url, start_time) VALUES ('$meeting_topic', '$join_url', '$meeting_start_time')";
        
        if (mysqli_query($connection, $store_meeting_query)) {
            echo json_encode(array('success' => true, 'join_url' => $join_url));
        } else {
            echo json_encode(array('error' => 'Failed to store meeting URL in the database.'));
        }
    } else {
        echo json_encode(array('error' => 'Failed to create Zoom meeting.'));
    }
        ...
        $('#zoomForm').on('submit', function (e) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        // Assuming you have a meeting ID returned or you can use it from the database
                        var meetingId = response.meeting_id;

                        $('#response').html(
                            '<div class="alert alert-success">Meeting created successfully!<br>' +
                            'Join Meeting: <a href="zoom_meeting.php?meeting_id=' + meetingId + '" target="_blank">Click here to join</a></div>'
                        );
                    } else {
